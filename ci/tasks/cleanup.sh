...
if [ "$BOSH_OPENSTACK_SSL_VERIFY" == 'false' ]; then
...
openstack_delete_entities() {
  ...
  local list_args=$2
  local delete_args=$3
  id_list=$(openstack $entity list $list_args $INSECURE --format json | jq --raw-output '.[].ID')
  for id in $id_list
  do
    echo "Deleting $entity $id ..."
    openstack $entity delete $delete_args $INSECURE $id
  done
}
...
do
  ...
  do
    ...
    openstack server remove volume $instance $volume $INSECURE
  done
  echo "Deleting server $instance ..."
  openstack server delete $instance --wait $INSECURE
done
