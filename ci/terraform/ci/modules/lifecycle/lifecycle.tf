...
output "lifecycle_no_dhcp_manual_ip_1" {
  value = cidrhost(
    openstack_networking_subnet_v2.lifecycle_subnet_no_dhcp_1.cidr,
    3,
  )
}
...
output "lifecycle_no_dhcp_manual_ip_2" {
  value = cidrhost(
    openstack_networking_subnet_v2.lifecycle_subnet_no_dhcp_2.cidr,
    3,
  )
}
...
resource "openstack_lb_listener_v2" "lifecycle_listener" {
  ...
  loadbalancer_id = element(
    openstack_lb_loadbalancer_v2.lifecycle_loadbalancer.*.id,
    count.index,
  )
  count = var.use_lbaas == "true" ? 1 : 0
}
...
resource "openstack_lb_pool_v2" "lifecycle_pool" {
  ...
  name      = "Lifecycle Pool"
  listener_id = element(
    openstack_lb_listener_v2.lifecycle_listener.*.id,
    count.index,
  )
  count = var.use_lbaas == "true" ? 1 : 0
}
