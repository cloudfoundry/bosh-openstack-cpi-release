...
describe Bosh::OpenStackCloud::ManualNetwork do
  ...
  describe '#prepare' do
    ...
    context "with 'use_nova_networking=false'" do
      ...
      context 'when IpAddressAlreadyAllocated error is raised' do
        let(:error) { Excon::Error::Conflict.new('some error message') }
        let(:network_spec) { manual_network_spec(net_id: 'net', ip: '10.0.0.1') }

        context 'when Excon::Error::Conflict is thrown but has not the type IpAddressAlreadyAllocated' do
          it 'will raise an Excon::Error::Conflict error with NotIpAddressAlreadyAllocated' do
        ...
        context 'when Excon::Error::Conflict is thrown and has not a NeutronError entry' do
          it 'will raise an Excon::Error::Conflict error' do
            allow(ports).to receive(:create).and_raise(error)
        ...
        context 'when conflicting ports exist but it has already been retried' do
          it 'will raise an Excon::Error::Conflict error and clean up conflicting ports' do
            allow(ports).to receive(:create).exactly(2).times.and_raise(error)
            allow(openstack).to receive(:parse_openstack_response).exactly(2).and_return('type' => 'IpAddressAlreadyAllocated')
            allow(ports).to receive(:all).exactly(2).times.and_return([])
        ...
        context 'when conflicting ports exist' do
          ...
          it 'will delete the conflicting ports and retry to create the ports for that IP exactly once' do
            retried = false
