...
describe Bosh::OpenStackCloud::FloatingIp do
  ...
  let(:network) { double('network', get_server: nil, list_floating_ips: nil, associate_floating_ip: nil, disassociate_floating_ip: nil, get_server_port_interfaces: nil, get_port: nil) }
  ...
  context 'when `use_nova_networking=false`' do
    ...
    let(:ports) {
        {
            floating_ip_port_id: 'old-server-port-id',
            interface_attachment_port_id: 'port-id',
      ...
      [
          {
              ...
              'status' => 'some-status',
              'port_id' => ports[:floating_ip_port_id],
              'id' => 'floating-ip-id'
          }
      ]
    }
    ...
    before(:each) {
      allow(network).to receive(:get_port).with(ports[:floating_ip_port_id]).and_return(get_port_response)
    }
    ...
    before(:each) {
      allow(network).to receive(:get_server).and_return(get_server_response)
    }

    let(:get_server_port_interfaces_response) {
      Struct.new(:body).new({
          'interfaceAttachments' => interface_attachments
      })
    }
    let(:interface_attachments) {
      [{
          'port_state' => 'ACTIVE',
          'fixed_ips' => [{
              'subnet_id' => 'some-subnet-id',
              'ip_address' => 'some-fixed-ip-address'
          }],
          'port_id' => ports[:interface_attachment_port_id],
          'net_id' => 'network-id',
          'mac_addr' => 'some-mac-address'
      }]
    }

    before(:each) {
      allow(network).to receive(:get_server_port_interfaces).and_return(get_server_port_interfaces_response)
    }
    ...
    describe '.reassociate' do
      ...
      context 'when the floating ip is already associated with a port' do
        ...
        it 'disassociates the floating ip and associates it with the given server' do
          ...
          expect(network).to have_received(:get_server_port_interfaces).with('server-id')
    ...
    describe '.disassociate_server' do

      before(:each) {
        allow(network).to receive(:list_floating_ips).and_return(floating_ips_response)
      }

      context 'when the server has a port with an associated floating ip' do
        let(:port_device_id) { server.id }

        let(:server) {
          double('server', id: 'server-id', name: 'server')
        }

        let(:ports) {
          {
              floating_ip_port_id: 'other-server-port-id',
              interface_attachment_port_id: 'other-server-port-id'
          }
        }

        it 'disassociates the floating ip' do
          Bosh::OpenStackCloud::FloatingIp.disassociate_server(openstack, server)

          expect(network).to have_received(:list_floating_ips)
          expect(network).to have_received(:get_server_port_interfaces).with('server-id')

          expect(network).to have_received(:disassociate_floating_ip).with('floating-ip-id')
          expect(logger).to have_received(:info).with("Disassociating floating IP '1.2.3.4' from server 'server (server-id)'")
        end
      end

      context 'when the server has no port (interface attachment)' do
        let(:interface_attachments) { [] }
        ...
        it 'does not disassociate the floating ip' do
          Bosh::OpenStackCloud::FloatingIp.disassociate_server(openstack, server)

          expect(network).to have_received(:list_floating_ips)
          expect(network).to have_received(:get_server_port_interfaces).with('server-id')

          expect(network).to_not have_received(:disassociate_floating_ip).with('floating-ip-id')
          expect(logger).to_not have_received(:info).with("Disassociating floating IP '1.2.3.4' from server 'server (server-id)'")
        end
      end

      context 'when the server has a port without an associated floating ip' do
        let(:interface_attachment_port_id) { 'non-matching-port-id' }
